...
RegisterNetEvent('snap-it:client:PedChecks', function()
    local PlyPed = PlayerPedId()
    if not IsPedInAnyVehicle(PlyPed) then
        local PlayerData = QBCore.Functions.GetPlayerData()
        if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] and not IsPauseMenuActive() then
            TriggerEvent('snap-it:client:takepicture')
        else
            QBCore.Functions.Notify('You can\'t do this action', 'error', 7500)
        end
    else
        QBCore.Functions.Notify('In a car bro?', 'error', 7500)
    end
end)

RegisterNetEvent("snap-it:client:takepicture", function()
    if not active then
        active = true
                    ...
                    while active and true do
                        ...
                        elseif IsControlJustPressed(1, 176) then
                            if not takethapicbro then
                                takethapicbro = true
                                QBCore.Functions.TriggerCallback("snap-it:server:WebHookCheck", function(hook)
                                    if hook then
                                        ...
                                        Wait(1350)
                                        QBCore.Functions.Notify('Uploading...', 'success', 1100)
                                        Wait(1850)
                                        QBCore.Functions.Notify('Photo uploaded!', 'success', 1400)
                                    else
                                        QBCore.Functions.Notify('Webhook Error', 'error', 7500)
                                    end
                                end)
                            end
                        end
